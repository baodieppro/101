...
+(LSPlayerView *)instancePlayerView{
    return [[[NSBundle mainBundle] loadNibNamed:@"LSPlayerView" owner:nil options:nil] lastObject];
}
...
- (void)handlePanBottomGesture:(UIPanGestureRecognizer*)gesture
{
    ...
    switch (gesture.state) {
        ...
        case UIGestureRecognizerStateBegan:
            ...
            break;
        case UIGestureRecognizerStateChanged: {
            LSLog(@"size==%@   拖拽位置   %f", NSStringFromCGPoint(self.center),point.y);
//            if(__kScreenWidth__ < point.x || point.x >0){
//            
//            }
        ...
        case UIGestureRecognizerStateFailed:
        {
            ...
            GSLog(@"lastCenter %@",NSStringFromCGPoint(lastCenter));
            if (lastCenter.x<0) {
                lastCenter.x=0;
            }
            ...
            CGRect vF = CGRectMake(0, 0, __kScreenWidth__, __kScreenHeight__);
            CGRect rect = CGRectIntersection([[[UIApplication sharedApplication]delegate] window].frame, vF);
...
//            if ([STCModel stcModel].isView == 1) {
                if (CellImageViewHeight > lastCenter.y) {
                    self.locationType = LSPlayerViewLocationTypeTop;
...
- (void)clickFullScreen
{
    ...
    if (self.isFullScreen) { //UIInterfaceOrientationPortrait
        [self setMyPrefersStatusBarHidden:NO];
        [self interfaceOrientation:UIInterfaceOrientationPortrait];
        
    }
    else {
        [self setMyPrefersStatusBarHidden:YES];
        [self interfaceOrientation:UIInterfaceOrientationLandscapeRight];
    }
}
#pragma mark - 隐藏顶部状态栏
-(void)setMyPrefersStatusBarHidden:(BOOL)isHidden{
    if ([self.delegate respondsToSelector:@selector(myPrefersStatusBarHidden:)]) {
        [self.delegate myPrefersStatusBarHidden:isHidden];
    }
}
...
-(void)gesture:(BOOL)isEnabled{
    
    if ([self topWindow].frame.size.width == __kScreenWidth__) {
        self.panGesture.enabled = isEnabled;
        self.pinchGesture.enabled = isEnabled;
    }else{
        self.panGesture.enabled = YES;
        self.pinchGesture.enabled = YES;
    }
...
- (void)stopAnimation
{
    if ([self topWindow].frame.size.width == __kScreenWidth__) {
        ...
        self.playerMaskView.totalTimeLabel.hidden = YES;
        self.playerMaskView.currentTimeLabel.hidden = YES;
    }
...
- (void)observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary<NSString*, id>*)change context:(void*)context
{
    ...
    if (self.isLoseActive)
        return;
    if(self.playerStatus == LSPlayerStatusStop){
//        [self closeClick];
        return;
    }
    if ([keyPath isEqualToString:@"status"]) {
        if ([self.playerItem status] == AVPlayerStatusReadyToPlay) { //准备播放
            ...
            if (self.locationType == LSPlayerViewLocationTypeTop) {
                [self topWindow].frame = CGRectMake(0, __kNavigationBarHeight__, [UIScreen mainScreen].bounds.size.width, CellImageViewHeight);
            }
...
- (void)updataPlayerViewBottomFrame
{
    CGSize size = [UIScreen mainScreen].bounds.size;
    //计算出tableView在window上的可视位置
    CGRect rect = CGRectIntersection([[[UIApplication sharedApplication]delegate] window].frame, self.tempSuperView.frame);
    [self topWindow].hidden = NO;
    [self topWindow].contentView = self;
...
- (void)showAlertView
{
    UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"当前网络为4G" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"继续", nil];
...
- (void)jump
{
    ...
    LSLog(@" jump %s", __func__);
}
- (void)failed
{
    LSLog(@"failed %s", __func__);
}
- (void)accessSuccess
{
    LSLog(@"accessSuccess  %s", __func__);
}
- (void)accessError
{
    LSLog(@"accessError  %s", __func__);
}
- (void)failedToPlay
{
    LSLog(@"failedToPlay  %s", __func__);
}
